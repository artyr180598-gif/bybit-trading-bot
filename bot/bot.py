...
EMA_SLOW     = 50
RSI_PERIOD   = 14
RSI_LONG_MIN = 45    # нужен реальный бычий импульс (было 38)
RSI_LONG_MAX = 65    # не входить в перекупленность (было 72)
RSI_SHORT_MIN= 30    # нужен реальный медвежий импульс
RSI_SHORT_MAX= 52    # не входить в шорт при RSI>52 — рынок ещё растёт (было 62)
ATR_PERIOD   = 14
ATR_SL_MULT  = 1.5
...
def get_signal(df4h, trend_1d):
    ...
    Индикаторы:
      ...
      RSI  — зона импульса (45-65 для LONG, 30-52 для SHORT)
...
def screen_strategy(cid):
    ...
    text = (
        ...
        f"  • SL: ATR×{ATR_SL_MULT} | TP: ATR×{ATR_TP_MULT} (R:R 1:1)\n"
        "  • 🔐 Безубыток при 50% пути к TP\n"
