...
import os, sys, time, json, random, string, logging, requests, math
import pandas as pd
import numpy  as np
...
# ─── АДАПТИВНЫЕ ПРОФИЛИ СТРАТЕГИЙ (отбор по бэктесту за 365 дней) ───────────
FEE_PCT          = 0.06   # Bybit taker fee (примерно) в % за сторону
SLIPPAGE_PCT     = 0.03   # консервативная проскальзывание в % за сторону
BACKTEST_DAYS    = 365
AUTO_OPT_ENABLED = os.environ.get("AUTO_STRATEGY_OPT", "true").lower() == "true"
ACTIVE_STRATEGY  = os.environ.get("STRATEGY_PROFILE", "balanced")

STRATEGY_PROFILES = {
    "balanced": {
        "label": "Balanced Trend",
        "rsi_long_min": 40, "rsi_long_max": 70, "rsi_short_min": 25, "rsi_short_max": 55,
        "adx_min": 20, "ema_sep_min_pct": 0.15, "confirm_bars": 2,
        "atr_sl_mult": 1.5, "atr_tp_mult": 1.5, "atr_tp2_mult": 3.0, "atr_trail": 1.0,
        "allow_flat_mode": True, "time_filter": True, "long_score_min": 3, "short_score_min": 3,
    },
    "momentum": {
        "label": "Momentum Breakout",
        "rsi_long_min": 50, "rsi_long_max": 78, "rsi_short_min": 22, "rsi_short_max": 50,
        "adx_min": 24, "ema_sep_min_pct": 0.20, "confirm_bars": 2,
        "atr_sl_mult": 1.4, "atr_tp_mult": 1.8, "atr_tp2_mult": 3.2, "atr_trail": 1.2,
        "allow_flat_mode": False, "time_filter": True, "long_score_min": 3, "short_score_min": 3,
    },
    "mean_reversion": {
        "label": "Mean Reversion",
        "rsi_long_min": 32, "rsi_long_max": 52, "rsi_short_min": 48, "rsi_short_max": 68,
        "adx_min": 14, "ema_sep_min_pct": 0.08, "confirm_bars": 1,
        "atr_sl_mult": 1.2, "atr_tp_mult": 1.3, "atr_tp2_mult": 2.1, "atr_trail": 0.9,
        "allow_flat_mode": True, "time_filter": False, "long_score_min": 3, "short_score_min": 3,
    },
    "defensive": {
        "label": "Defensive Quality",
        "rsi_long_min": 45, "rsi_long_max": 68, "rsi_short_min": 28, "rsi_short_max": 52,
        "adx_min": 26, "ema_sep_min_pct": 0.22, "confirm_bars": 3,
        "atr_sl_mult": 1.3, "atr_tp_mult": 1.6, "atr_tp2_mult": 2.8, "atr_trail": 1.1,
        "allow_flat_mode": False, "time_filter": True, "long_score_min": 3, "short_score_min": 3,
    },
}
...
def fetch_klines_history(symbol, interval="60", days=365):
    """Исторические свечи за N дней через Bybit (с пагинацией)."""
    start_ms = int((datetime.now(timezone.utc) - timedelta(days=days)).timestamp() * 1000)
    end_ms = int(datetime.now(timezone.utc).timestamp() * 1000)
    cursor_end = end_ms
    chunks = []

    for _ in range(8):  # 8*1000 свечей по 1H ~= 333 дней; плюс частичное перекрытие
        try:
            r = requests.get(
                f"{BYBIT_URL}/v5/market/kline",
                params={
            ...
            if not rows:
                break
            df_chunk = pd.DataFrame(rows, columns=["ts", "op", "hi", "lo", "cl", "vol", "turnover"])
            df_chunk = df_chunk.astype({c: float for c in df_chunk.columns})
            chunks.append(df_chunk)
            min_ts = int(df_chunk["ts"].min())
            if min_ts <= start_ms:
                break
            cursor_end = min_ts - 1
            time.sleep(0.15)
    ...
    # Фолбэк: OKX history candles (обычно доступно даже если Bybit блокируется)
    try:
        inst = _okx_symbol(symbol)
        bar = _okx_bar(interval)
        all_rows = []
        before = None
        for _ in range(40):  # 40*300 1H свечей покрывает >365 дней
            params = {"instId": inst, "bar": bar, "limit": 300}
            if before:
                params["before"] = before
            r = requests.get("https://www.okx.com/api/v5/market/history-candles", params=params, timeout=20)
            data = r.json().get("data", [])
            if not data:
                break
            all_rows.extend(data)
            before = data[-1][0]
            if float(before) <= start_ms:
                break
            time.sleep(0.12)
        if not all_rows:
            return None
...
def get_profile(name=None):
    prof_name = (name or ACTIVE_STRATEGY or "balanced").strip().lower()
    return STRATEGY_PROFILES.get(prof_name, STRATEGY_PROFILES["balanced"])


def _signal_with_profile(df4h, trend_1d, profile):
    if len(df4h) < 3:
        return None
    c = df4h.iloc[-1]
    p = df4h.iloc[-2]

    ema_sep_pct = abs(c["ema_mid"] - c["ema_slow"]) / max(c["cl"], 1e-9) * 100
    if ema_sep_pct < profile["ema_sep_min_pct"]:
        return None
    if profile["adx_min"] > 0 and c.get("adx", 100) < profile["adx_min"]:
        return None
    if profile["time_filter"]:
        h = datetime.fromtimestamp(c["ts"] / 1000.0, tz=timezone.utc).hour
        if 0 <= h < 5:
            return None
    ...
    st_bull   = c["st_dir"] == 1
    st_bear   = c["st_dir"] == -1
    rsi_long  = profile["rsi_long_min"] <= c["rsi"] <= profile["rsi_long_max"]
    rsi_short = profile["rsi_short_min"] <= c["rsi"] <= profile["rsi_short_max"]
    ...
    sig = None
    if long_score >= profile["long_score_min"] and trend_1d > 0:
        sig = "LONG"
    elif short_score >= profile["short_score_min"] and trend_1d < 0:
        sig = "SHORT"
    elif profile["allow_flat_mode"] and long_score == 4 and trend_1d == 0:
        sig = "LONG"
    elif profile["allow_flat_mode"] and short_score == 4 and trend_1d == 0:
        sig = "SHORT"
    if not sig:
        return None

    if profile["confirm_bars"] >= 2 and len(df4h) >= 3:
        p2 = df4h.iloc[-3]
        prev_long_score = sum([
            p["ema_mid"] > p["ema_slow"], p["st_dir"] == 1,
            profile["rsi_long_min"] <= p["rsi"] <= profile["rsi_long_max"],
            p["macd_h"] > 0 and p["macd_h"] > p2["macd_h"],
        ])
        prev_short_score = sum([
            p["ema_mid"] < p["ema_slow"], p["st_dir"] == -1,
            profile["rsi_short_min"] <= p["rsi"] <= profile["rsi_short_max"],
            p["macd_h"] < 0 and p["macd_h"] < p2["macd_h"],
        ])
        if sig == "LONG" and prev_long_score < profile["long_score_min"]:
            return None
        if sig == "SHORT" and prev_short_score < profile["short_score_min"]:
            return None
    return sig
...
def get_signal(df4h, trend_1d):
    ...
    """
    return _signal_with_profile(df4h, trend_1d, get_profile())
...
def _trend_from_daily(df_daily, idx):
    row = df_daily.iloc[idx]
    if row["cl"] > row["ema_mid"] and row["ema_mid"] > row["ema_slow"]:
        return 1
    if row["cl"] < row["ema_mid"] and row["ema_mid"] < row["ema_slow"]:
        return -1
    return 0
...
def backtest_profile(df1h, df1d, profile, initial_capital=10000.0):
    equity = initial_capital
    peak = initial_capital
    max_dd = 0.0
    pos = None
    trades = []
    fee_factor = (FEE_PCT + SLIPPAGE_PCT) / 100.0
    day_map = df1d.set_index(df1d["ts"]).sort_index()

    for i in range(120, len(df1h)):
        window = df1h.iloc[:i+1]
        c = window.iloc[-1]
        ts_now = c["ts"]
        daily_ts = day_map.index[day_map.index <= ts_now]
        if len(daily_ts) == 0:
            continue
        trend = _trend_from_daily(df1d[df1d["ts"] <= daily_ts[-1]], -1)
        ...
        if pos:
            ...
            if pos["side"] == "LONG":
                hit_sl = price <= pos["sl"]
                hit_tp = price >= pos["tp"]
                exit_signal = check_exit_signal(window, "LONG")
                raw = (price - pos["entry"]) * pos["qty"] if (hit_sl or hit_tp or exit_signal) else None
            else:
                hit_sl = price >= pos["sl"]
                hit_tp = price <= pos["tp"]
                exit_signal = check_exit_signal(window, "SHORT")
                raw = (pos["entry"] - price) * pos["qty"] if (hit_sl or hit_tp or exit_signal) else None
            if raw is not None:
                fees = (pos["entry"] * pos["qty"] + price * pos["qty"]) * fee_factor
                pnl = raw - fees
                equity += pnl
        ...
        if pos is None:
            sig = _signal_with_profile(window, trend, profile)
            if not sig:
                continue
            atr = c["atr"]
            risk_cash = equity * (RISK_PCT / 100.0)
            sl_mult = profile["atr_sl_mult"]
            risk_unit = max(atr * sl_mult, c["cl"] * 0.002)
            qty = risk_cash / max(risk_unit, 1e-9)
            if sig == "LONG":
                sl = c["cl"] - atr * sl_mult
                tp = c["cl"] + atr * profile["atr_tp_mult"]
            else:
                sl = c["cl"] + atr * sl_mult
                tp = c["cl"] - atr * profile["atr_tp_mult"]
            pos = {"side": sig, "entry": c["cl"], "qty": qty, "sl": sl, "tp": tp}

    if not trades:
        return {"return_pct": -100.0, "trades": 0, "win_rate": 0.0, "max_dd": 100.0, "score": -999}
    wins = sum(1 for p in trades if p > 0)
    ret = (equity / initial_capital - 1.0) * 100
    wr = wins / len(trades) * 100
    pnl_series = pd.Series(trades, dtype=float)
    sharpe = (pnl_series.mean() / (pnl_series.std() + 1e-9)) * math.sqrt(365) if len(trades) > 5 else 0
    score = ret - max_dd * 1.2 + sharpe * 3 + max(0, wr - 45) * 0.5
    return {"return_pct": ret, "trades": len(trades), "win_rate": wr, "max_dd": max_dd, "score": score}


def optimize_strategy_profiles():
    global ACTIVE_STRATEGY
    if not AUTO_OPT_ENABLED:
        return
    logger.info("🔬 Запускаем отбор профиля стратегии за последние %d дней...", BACKTEST_DAYS)
    rows = []
    for name, profile in STRATEGY_PROFILES.items():
        metrics = []
        for pair in PAIRS[:5]:
            sym = pair["symbol"]
            df1h = fetch_klines_history(sym, "60", BACKTEST_DAYS)
            df1d = fetch_klines_history(sym, "D", BACKTEST_DAYS)
            if df1h is None or df1d is None:
                continue
            df1h = calc_indicators(df1h)
            df1d = calc_indicators(df1d)
            m = backtest_profile(df1h, df1d, profile)
            metrics.append(m)
        if not metrics:
            continue
        avg_ret = float(np.mean([m["return_pct"] for m in metrics]))
        avg_dd = float(np.mean([m["max_dd"] for m in metrics]))
        avg_wr = float(np.mean([m["win_rate"] for m in metrics]))
        avg_tr = float(np.mean([m["trades"] for m in metrics]))
        avg_score = float(np.mean([m["score"] for m in metrics]))
        rows.append((name, avg_score, avg_ret, avg_wr, avg_dd, avg_tr))
        logger.info("Profile %-14s | score=%7.2f ret=%6.2f%% wr=%5.1f%% dd=%5.2f%% trades=%5.1f",
                    name, avg_score, avg_ret, avg_wr, avg_dd, avg_tr)

    if not rows:
        logger.warning("Авто-оптимизация не смогла загрузить исторические данные.")
        return
    rows.sort(key=lambda x: x[1], reverse=True)
    ACTIVE_STRATEGY = rows[0][0]
    logger.info("🏆 Выбран профиль стратегии: %s (%s)", ACTIVE_STRATEGY, get_profile(ACTIVE_STRATEGY)["label"])
...
def do_open(pair, price, atr, side):
    ...
    prof = get_profile()
    if side == "LONG":
        sl  = price - prof["atr_sl_mult"]  * atr
        tp1 = price + prof["atr_tp_mult"]  * atr   # первая цель: закрыть PARTIAL_PCT
        tp2 = price + prof["atr_tp2_mult"] * atr   # вторая цель: закрыть остаток
    else:
        sl  = price + prof["atr_sl_mult"]  * atr
        tp1 = price - prof["atr_tp_mult"]  * atr
        tp2 = price - prof["atr_tp2_mult"] * atr
...
def update_trailing(pair, price):
    ...
    # ── Трейлинг-стоп ─────────────────────────────────────────────────────────
    trail_mult = get_profile().get("atr_trail", ATR_TRAIL)
    if side == "LONG":
        new_sl = round(price - trail_mult * atr, 2)
...
def screen_main(cid):
    try:
        ...
        text = (
            ...
            f"📊 Стратегия: {get_profile()['label']} ({ACTIVE_STRATEGY})\n"
...
def screen_strategy(cid):
    ...
    text = (
        f"📈 <b>Стратегия бота — {prof['label']}</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
        ...
        f"  • RSI — импульс (LONG: {prof['rsi_long_min']}–{prof['rsi_long_max']}, SHORT: {prof['rsi_short_min']}–{prof['rsi_short_max']})\n"
        "  • MACD — гистограмма на правильной стороне нуля\n\n"
        "⚙️ <b>Защита капитала:</b>\n"
        f"  • Риск: {RISK_PCT}% на сделку | Плечо: {LEVERAGE}x\n"
        f"  • SL: ATR×{prof['atr_sl_mult']} | TP: ATR×{prof['atr_tp_mult']}\n"
    ...
    if TOKEN:
        ...
        if ADMIN_ID:
            ...
            send(ADMIN_ID,
                 ...
                 f"🎯 Цель: устойчивый риск-контроль и положительное матожидание\n"
                 f"🕐 {ts()}")
