...
ATR_PERIOD   = 14
ATR_SL_MULT  = 1.5
ATR_TP_MULT  = 2.5    # расширен с 1.5 → даёт цене дойти до TP (R1 оптимизация)
ATR_TRAIL    = 1.0
BE_TRIGGER   = 0.0    # ОТКЛЮЧЁН: BE срезал прибыль (avg win c $1.44 → $5.70)
BE_BUFFER    = 0.15   # небольшой буфер выше/ниже входа (в ATR) при переводе в безубыток
EMA_SEP_MIN_PCT = 0.20  # мин. разрыв EMA21/EMA50 в % от цены — фильтр шума/боковика (R2 оптимизация)
ST_MULT      = 3.0
ST_PERIOD    = 10
MACD_FAST    = 12
MACD_SLOW    = 26
...
def get_signal(df4h, trend_1d):
    ...
    # Фильтр силы тренда: EMA21 и EMA50 должны быть достаточно далеко друг от друга.
    # Если EMAs почти совпадают — рынок в боковике, входить опасно (много ложных сигналов).
    # Порог 0.20% от цены — оптимизировано бэктестом (R2), даёт +0.53% к результату.
    ...
    # Это позволяет боту торговать в любом рынке, но очень избирательно
    if long_score == 4 and trend_1d == 0:
        return "LONG"
    if short_score == 4 and trend_1d == 0:
        return "SHORT"
    return None
